...
export async function sendTTSMessage(text: string, model: string, voice: string, conversationId: number) {
  ...
  try {
    const response = await fetch("https://gptapichat.online/v1/audio/speech", {
      method: "POST",
      headers: {
...
let finalMessages = [...cleansedMessages, currentMessage];
  ...
  let source = new SSE("https://gptapichat.online/v1/chat/completions", {
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${get(apiKey)}`,
    },
...
console.log(msg);
  ...
  export async function sendDalleMessage(msg: ChatCompletionRequestMessage[], convId) {
    ...
    try {
      let response = await fetch("https://gptapichat.online/v1/images/generations", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${get(apiKey)}`,
